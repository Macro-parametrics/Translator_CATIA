...
namespace Pre {
	...
	void AttLinearRepartation::GetInfo(ifstream &is)
	{
		...
		LengthValue = strtok_s(NULL, seps_temp, &context);	//세번째 단어 : rectPattern1
		_featureName = LengthValue;

		LengthValue = strtok_s(NULL, seps_temp, &context);	//네번째 단어 : FirstDirectionRepartition
		_attributeName = LengthValue;

		context = NULL;
	}

	void AttLinearRepartation::Modify(char * buffer)  //Script 파일에 바로 Class 인스턴스 이름이 나올때 
	{
	}
